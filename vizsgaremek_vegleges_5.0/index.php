...
#menu-left .dropdown{ position:relative; display:inline-block; margin-right:8px; }
...
#menu-left .dropbtn .caret{ margin-left:8px; font-size:12px; color:#ffa500; }

#menu-left .dropdown-menu{
  position:absolute; top:100%; left:0; z-index:2000;
  display:none; padding:8px; margin-top:8px;
  background:#444;               
  color:#ffa500;
  border:1px solid #444;
  border-radius:12px;
  box-shadow:0 10px 28px #444;
  min-width:220px; width:max-content;
}
#menu-left .dropdown.open .dropdown-menu{ display:block; }

#menu-left .dropdown-menu a{
  display:block; padding:9px 12px; border-radius:8px;
  text-decoration:none; color:#ffa500 !important;  
  font-weight:500;
}
#menu-left .dropdown-menu a:hover{
  background:#1c1c1c;           
...
<div id="login">
    <div id="menu-left">
        ...
        <a href="./?p=adomanyozz"><button>Adományozz!</button></a>
        <a href="./?p=esemenyek"><button>Események</button></a>
     <div class="dropdown" id="free-dd">
  <button id="free-dd-btn" class="dropbtn">Ingyen könyvek <span class="caret">▾</span></button>
  <div class="dropdown-menu">
    <a href="./?p=ingyen_konyvek">Összes ingyen könyv</a>
    <?php if (isset($_SESSION['uid'])): ?>
      <a href="./?p=ingyen_konyv_feladas">+ Felajánlás</a>
      <a href="./?p=ingyen_konyveim">Felajánlásaim</a>
      <a href="./?p=ingyen_konyv_kereseim">Igényléseim</a>
    <?php else: ?>
      <a href="./?p=login">Bejelentkezés a felajánláshoz</a>
    <?php endif; ?>
...
<?php
   ...
   if (!isset($_SESSION['uid'])) {
    if ($p == "")                       include("kezdolap.php");
        ...
        else if ($p == "adomanyozz")        include("adomanyozz.php");    
        else if ($p == "ingyen_konyvek")        include("ingyen_konyvek.php");
        else if ($p == "ingyen_konyv_igenyles") include("ingyen_konyv_igenyles.php");
        else if ($p == "ingyen_konyv_feladas")  include("ingyen_konyv_feladas.php");
        else if ($p == "ingyen_konyveim")       include("ingyen_konyveim.php");
        else if ($p == "ingyen_konyv_kereseim") include("ingyen_konyv_kereseim.php");
        else if ($p == "reg_org")          include("reg_org.php");   
        else if ($p == "esemenyek") include("esemenyek.php");
...
<footer>
    <p style="color: white; text-align: center; ">&copy; 2025 Bookli.hu Minden jog fenntartva. Hogy hivják a kínai könyvmolyt. Bookly! </p>
</footer>
<script>
  (function(){
    const dd  = document.getElementById('free-dd');
    const btn = document.getElementById('free-dd-btn');
    if(!dd || !btn) return;
